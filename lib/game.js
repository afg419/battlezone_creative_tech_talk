...
class Game{
  ...
  updateMothershipState(){
    this.level.mothership.forEach( shipBody => {
      if(shipBody.inDetonationRange(this.player)){
        shipBody.detonate(this.player);
      }
    });
  }
  ...
  updateTurretState(){
    this.level.turrets.forEach( (turret, index, turretArray) => {
      turret.timer ++;
      if(turret.inFireRange(this.player) && turret.timer > 5){
