...
class Player{
  ...
  rotateBy(deg){
    let newX = Math.cos(deg)*this.head.x - Math.sin(deg)*this.head.y;
    let newY = Math.sin(deg)*this.head.x + Math.cos(deg)*this.head.y;
    this.head = new Vector(newX, newY, 0);
  }

  rotateVertBy(deg){
    this.head = this.head.rotateVert(deg)
    // this.head = new Vector(newX, newY, 0);
  }
  ...
  scaleU(){
    return windowheight/(2 * this.maxY())
  }

  scaleV(){
    return windowwidth/(2 * this.maxZ())
  }
}
