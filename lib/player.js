...
class Player{
  ...
  makeBullet(side){
    var location = this.loc.plus(Maths.scalMult(5, this.head)).plus(Maths.scalMult(side * 4, this.u)).minus(Maths.scalMult(4, this.v));
    var pre = this.head;//.plus(this.momentum);
    var heading = Maths.scalMult(1/pre.mag(), pre);
    return new Bullet(location, heading, (this.speed + 25), 1, this.u);
  }
