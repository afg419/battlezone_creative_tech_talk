...
class ObstacleBuilder{
  ...
  mineObstacle(loc, width, mineColor = [204,0,0]){
  ...
  turretObstacle(loc, width){
    var bootyColor = [170,170,170];
    var frontBottomLeft = loc.plus(new Vector(width/2, -width/2, -width/2));
    var backTopRight = loc.plus(new Vector(-width/2, width/2, width/2));
    var faces = [];
      faces[0] = this.square(frontBottomLeft, new Vector(0,1,0), new Vector(0,0,1), width, bootyColor);
  ...
  bulletObstacle(loc, heading, u) {
    ...
    var faces = [];
    var edgeArrow1 = Maths.scalMult(7, heading).plus(Maths.scalMult(0.2,u));
    var edgeArrow2 = Maths.scalMult(7, heading).plus(Maths.scalMult(-0.2,u));
